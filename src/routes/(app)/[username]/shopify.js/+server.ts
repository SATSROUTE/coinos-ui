...
// Mesma regra do servidor (coinos-server lib/register.ts): so letras e numeros,
// de 2 a 24. Um usuario legitimo nunca tem aspas no nome, entao o que nao casa
// aqui e tentativa de injetar codigo no script gerado.
const validUsername = /^[\p{L}\p{N}]{2,24}$/u;

export async function GET({ params, url }) {
	const { username } = params;

	if (!validUsername.test(username)) error(404, "Not found");
	...
	let data = await fetch('https://${url.host}/api/invoice', {
		...
		body: JSON.stringify({
			invoice: {
				fiat,
				type: 'lightning',
				webhook: ${JSON.stringify(`https://${url.host}/api/shopify/`)} + Shopify.checkout.order_id
			},
