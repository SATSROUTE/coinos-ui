...
window.addEventListener('load', async () => {
  ...
  await wait(() => ["satsroute", "coinos", "bitcoin", "btc", "btcpayserver", "btcpay server"].some(v => ~(document.querySelector(".payment-method-list__item__info").innerText.toLowerCase().indexOf(v))));
	...
	let data = await fetch('https://preview.satsroute.com/api/invoice', {
		method: 'POST',
		mode: 'cors',
		headers: {
		...
		body: JSON.stringify({
			invoice: {
				fiat,
				type: 'lightning',
				webhook: "https://preview.satsroute.com/api/shopify/" + Shopify.checkout.order_id
			},
			user: { username: "srtest5898" }
		})
	})
	...
	let ws = new WebSocket('wss://preview.satsroute.com/ws');
	ws.onmessage = ({ data }) => {
		try {
			let { type, data: d } = JSON.parse(data);
			if (type === 'payment') {
				main.innerHTML =
					`<div style="display: flex"><img id="check" src="https://preview.satsroute.com/icons/check.svg" alt="Check" style="margin: auto; max-width: 300px" /></div>
